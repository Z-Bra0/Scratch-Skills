...
class ConnectorSpec:
    prefix_levels: int
    left_type: ConnectorType
    first_span: int
    middle_type: ConnectorType | None
    second_span: int
    right_type: ConnectorType
...
def connector_spec(level1: int, width1: int, level2: int, width2: int) -> ConnectorSpec:
    ...
    if level1 == 0 and width1 == 0:
        return ConnectorSpec(0, ConnectorType.DOWN, width2, None, 0, ConnectorType.END_DOWN)
    if level2 == 0 and width2 == 0:
        return ConnectorSpec(0, ConnectorType.UP, width1, None, 0, ConnectorType.END_UP)

    delta_level = level2 - level1
    prefix_levels = level1 if delta_level == 0 else min(level1, level2) + 1
    real_width1 = width1 + level1 * len(LVL_PREFIX)
    real_width2 = width2 + level2 * len(LVL_PREFIX)

    if delta_level == 1:
        left = ConnectorType.DOWN
    elif delta_level == -1:
        left = ConnectorType.UP
    else:
        left = ConnectorType.FLAT

    if width1 != 0 and real_width1 < real_width2:
        middle = ConnectorType.JOIN_DOWN
    elif width2 != 0 and real_width1 > real_width2:
        middle = ConnectorType.JOIN_UP
    else:
        middle = None

    prefix_width = prefix_levels * len(LVL_PREFIX)
    first_span = min(real_width1, real_width2) - prefix_width
    second_span = abs(real_width1 - real_width2) - (1 if middle is not None else 0)

    if real_width1 < real_width2:
        right = ConnectorType.END_DOWN
    elif real_width1 > real_width2:
        right = ConnectorType.END_UP
    else:
        right = ConnectorType.END_FLAT

    return ConnectorSpec(prefix_levels, left, first_span, middle, second_span, right)


def render_connector_spec(spec: ConnectorSpec, horizontal: str = "─") -> str:
    return (
        (LVL_PREFIX * spec.prefix_levels)
        + spec.left_type.value
        + (horizontal * spec.first_span)
        + (spec.middle_type.value if spec.middle_type is not None else "")
        + (horizontal * spec.second_span)
        + spec.right_type.value
    )


def connector_line(level1: int, width1: int, level2: int, width2: int) -> str:
    return render_connector_spec(connector_spec(level1, width1, level2, width2))
